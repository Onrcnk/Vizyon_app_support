# SmartBudget Uygulaması – Hesap ve Veri Silme Talebi
SmartBudget uygulamasında oluşturduğunuz hesabınızı ve ilişkili tüm verilerinizi silmek için aşağıdaki adımları takip edebilirsiniz.


👤 Geliştirici Adı:
Vizyon Yazılım / SmartBudget


📝 Hesap Silme Adımları

Lütfen aşağıdaki bilgileri içeren bir silme talebini, kayıtlı e-posta adresinizden göndererek bize iletin:

Adınız ve soyadınız

Uygulamada kullandığınız e-posta adresi

“Hesabımın ve tüm verilerimin silinmesini talep ediyorum.” ifadesi


📧 Silme talepleri için e-posta adresi: vizyon.smartbudget@gmail.com


📦 Silinen ve Saklanan Veriler
Silinen Veriler:

Kullanıcı hesabı bilgileri (isim, e-posta)

Uygulama içi finansal veriler

Tercihler ve kişisel ayarlar

Saklanan Veriler:

🕒 Silme Süreci
Silme talebinizin alınmasının ardından hesabınız ve ilişkili verileriniz en geç 14 iş günü içinde sistemden kalıcı olarak silinecektir.
Silme işlemi geri alınamaz.
